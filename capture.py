...
from myTeam import DummyAgent, DefensiveReflexAgent, OffensiveReflexAgent
...
def plot_q(weights):
  ...
  plot q values
  """
  cmp = plt.cm.get_cmap('hsv', len(weights.keys()))
  
  # plot metrics
  for idx, (f, W) in enumerate(weights):
    plt.plot(W, color=cmp(idx), lw=2, label='{}'.format(f))
